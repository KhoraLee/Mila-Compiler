...
std::shared_ptr<Token> Lexer::next_token() {
  ...
  switch(current) {
  case EOF:
    return make_token<BasicToken>(TOK_EOF);
  case 'a' ... 'z':
  case 'A' ... 'Z': {
    const std::string identifier = read_identifier();
    auto it = keyword_map.find(identifier);
    if (it != keyword_map.end()) {
      return make_token<BasicToken>(it->second);
    }
    it = operator_map.find(identifier);
    if (it != operator_map.end()) {
      return make_token<OperatorToken>(it->second);
    }
    return make_token<IdentifierToken>(identifier);
  }
  // number
  case '0' ... '9': {
    bool is_float = false;
    ...
    if (is_float) {
      return make_token<FloatToken>(num);
    } else {
      return make_token<IntToken>(num);
    }
  }
  case '&': // oct
    return make_token<IntToken>(read_num(IntBase::OCT));
  case '$': // hex
    return make_token<IntToken>(read_num(IntBase::HEX));
  case '\'': // string
    return make_token<StringToken>(read_string());
  case '<':
  case '=':
  case '>':
  case '+':
  ...
  case ':': {
    ...
    if (it2 != symbol_map.end() && op.length() == 1) {
      return make_token<BasicToken>(it2->second);
    }
